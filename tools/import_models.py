#!/usr/bin/env python3
"""Repack retained CC0 photoscan sources with Blender/Python, without downloads.

The existing cache in tools/downloads/models must contain source glTFs, textures,
alpha maps and metadata. Missing or changed files fail closed; the shipped models
are sufficient to play and are not regenerated by builds. Blender exports separate
uncompressed glTF geometry and complete PNG PBR maps, without atlas baking or
joining variant/LOD objects. Optional simplification uses Blender Decimate, not
meshoptimizer; a reconverted asset needs visual review before it replaces a release.
Conversions are staged and validated before promotion; previous files are retained
in local-data/model-imports. Unselected SOURCES.md provenance stays verbatim.

Usage:
    python3 tools/import_models.py --only fern_02,rock_07
    python3 tools/import_models.py --credits-only
"""
...
def fetch(url: str, dest: pathlib.Path, md5: str | None = None) -> pathlib.Path:
    if cache_matches(dest, md5):
        return dest
    raise FileNotFoundError(f"Retained source is missing or does not match its recorded hash: {dest} ({url}); no downloads are performed")
...
def load_cached_sources(entry: Entry) -> pathlib.Path:
    ...
    # Reading a cache is not a new retrieval; preserve its historical receipt.
    return compose_cutouts(entry, files, sources[0][1])
...
def compose_cutouts(entry: Entry, files: dict, gltf: pathlib.Path) -> pathlib.Path:
    ...
    When the asset publishes a cached alpha (or mask) map, put it in the
...
def convert(entry: Entry, gltf: pathlib.Path, out: pathlib.Path) -> None:
    ...
    args = ["blender", "--background", "--factory-startup", "--threads", "2",
            "--python-exit-code", "1", "--python", str(ROOT / "tools/convert_model_blender.py"), "--",
            "--input", str(gltf.resolve()), "--output", str(staged_out),
            "--texture-size", str(entry.texture_size), "--simplify-ratio", str(entry.simplify_ratio)]
...
def alterations(entry: Entry) -> str:
    parts = [f"glTF {entry.res} repacked with Blender/Python (uncompressed geometry; separate objects and LOD nodes; external PNG PBR textures, including composed cutout alpha)",
             f"textures capped at {entry.texture_size} px"]
    if entry.simplify_ratio > 0:
        parts.append(f"Blender Decimate collapse ratio {entry.simplify_ratio:.3f}; not the earlier meshoptimizer recipe")
    return "; ".join(parts) + "."
...
def write_sources(names: list[str], refreshed: set[str] | None = None,
    ...
    for name in names:
        ...
        if name in converted:
            recipe = alterations(entry)
        elif name in retained:
            recipe = retained[name].split("|")[8].strip()
        else:
            raise RuntimeError(f"No recorded conversion provenance for {name}; credits alone cannot establish how the model was made")
        rows.append(f"| `{name}/` | [{info.get('name', entry.asset)}](https://polyhaven.com/a/{entry.asset}) | {authors} | Poly Haven | {LICENSE} | {retrieved} | {tris:,} tris | {recipe} |")
    (MODELS / "SOURCES.md").write_text(
        ...
        "retained with its original conversion provenance below; `tools/import_models.py` can repack cached sources with Blender. "
...
def main() -> int:
    ...
    parser.add_argument("--credits-only", action="store_true", help="preserve retained credit rows; --only refreshes selected metadata without changing conversion provenance")
    ...
    if not args.credits_only:
        if shutil.which("blender") is None:
            parser.error("Blender is required on PATH (docs/development.md)")
        for name in names:
            entry = MANIFEST[name]
            print(f"{name} <- polyhaven/{entry.asset} @ {entry.res}", flush=True)
            gltf = load_cached_sources(entry)
            out = MODELS / name / f"{name}.gltf"
    ...
    write_sources(list(MANIFEST), refreshed, converted=converted)
